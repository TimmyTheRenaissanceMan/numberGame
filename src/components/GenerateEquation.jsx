...
export default class GenerateEquation {
  ...
  determineMaxLength() {
    let maxResultLength;
    console.log(store.getState().arithmeticSigns.length);
    if (store.getState().arithmeticSigns.length > 2) {
      switch (true) {
        case this.length === 5:
          maxResultLength = 1;
          break;
  ...
  generateEquation(targetResult, maxResultLength) {
    ...
    if (!passed) {
      this.stack++;
      this.determineResultLength(maxResultLength);
    } else {
  ...
  limitedSigns() {
    ...
    let sign = store.getState().arithmeticSigns[
      Math.floor(
        Math.random() * (store.getState().arithmeticSigns.length - 1 + 1) + 0
      )
    ];
    let sign2 = store.getState().arithmeticSigns[
      Math.floor(
        Math.random() * (store.getState().arithmeticSigns.length - 1 + 1) + 0
      )
    ];
    ...
    if (sign === "/" || sign2 === "/") {
      ...
      console.log(varA, sign, varB, sign2, varC);
      return this.calc(varA, sign, varB, sign2, varC);
    }
  }
    ...
    switch (true) {
      ...
      case curLength === this.length && output > 0 && output % 1 === 0:
        if (round === 1) {
          ...
          return a.toString() + sign + b.toString() + "=" + output.toString();
        } else {
          return (
            a.toString() +
            sign +
